...
import { getTokenHolders } from "../../../api/Token"

import { ITokenHoldersResponse } from "../../../interfaces/tokens/TokenHoldersResponse"
...
export default function Info() {
  ...
  const [tokenHoldersCount, setTokenHoldersCount] =
    useState<ITokenHoldersResponse>()
  ...
  async function fetchInfo() {
    try {
      ...
      const name_ = await getName()
      if (isMountedRef.current) {
        dispatch(setName(name_))
        setLoadingName(false)
      }

      const symbol_ = await getSymbol()
      if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    getTokenHolders()
      .then((response) => {
        setTokenHoldersCount(response?.data)
      })
      .catch((error) => {
        console.error("Error getting the holders data", error?.response)
      })
            ...
            <InfoRow>
              ...
              <InfoValue>
                {loadingSymbol ? (
                  ...
                  tokenHoldersCount?.records.length ?? 0
                )}
              </InfoValue>
            </InfoRow>
          </InfoCell>
