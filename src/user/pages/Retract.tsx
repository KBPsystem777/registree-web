...
import { retractNft } from "../../blockchain/actions/MLNftAction"
import { logNftTransaction } from "../../api/Nft"
import { ethers } from "ethers"
import { useModalContext } from "../../hooks/ModalContext"
import { errorCatcher } from "../../utils/helpers"

export default function Retract({ nft, closeModal }: any) {
  ...
  async function handleNftRetraction() {
    try {
      if (account) {
        setLoading(true)
        const tx = await retractNft(nft.property_id || nft.propertyId)
        if (tx) {
          console.log(tx)
          ...
          console.log(res)
          setSuccessMessage("Revoking of NFT succeed.")
        } else {
          setErrorMessage("Revoking of NFT failed.")
        }
      ...
      // @todo - Make this method work: setErrorMessage(errorCatcher(e))
      setLoading(false)
    }
  }
    ...
    >
      ...
      <RowJustified>
        <Button
          ...
          bg="#2A72A7"
          disabled={loading}
          onClick={() => (loading ? null : handleNftRetraction())}
        >
          Revoke NFT
        </Button>
        <Button
