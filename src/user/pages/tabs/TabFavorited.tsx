...
import { getNftFavorited } from "../../../api/Nft";
...
let w = 14,
  h = 18;
export default function TabFavorited() {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (account) {
          setLoading(true);
          const raw = await getNftFavorited();
          console.log(raw);
          if (raw?.data?.data) {
  ...
  function handleShowDetailClick(item: any) {
    const nftDetailRoute = location.pathname.includes("/collections")
      ? "collections"
      : "marketplace";
    navigate(
      `/${nftDetailRoute}/${item?.token_address || ADDRESS}/${item?.token_id || item?.nft?.propertyId}`
    );
  }
    ...
    <Container>
      ...
      <NftGrid>
        {loading ? (
          ...
          items.map((item, i) => {
            return (
              <NftBox key={i} onClick={() => handleShowDetailClick(item)}>
                <NftImage
                  src={item?.nft?.image || NoImageIcon}
                  hasImage={item?.nft?.image}
                />
                <Attrs>
                  <PropertyName>
                    {item?.nft?.name || item?.nft?.propertyId}
                  </PropertyName>
                  <InfoIcon
