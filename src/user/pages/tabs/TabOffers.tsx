...
import { getNftOffered, getUserNftOffersFromAdmin } from "../../../api/Nft";
...
export default function TabOffers() {
    ...
    useEffect(() => {
        (async () => {
            try {
                if (account) {
                    setLoading(true);
                    const raw2 = await getUserNftOffersFromAdmin();
                    console.log(raw2);
                    const raw = await getNftOffered();
    ...
    function handleShowDetailClick(item: any) {
        ...
        navigate(`/${nftDetailRoute}/${item?.token_address}/${item?.token_id}`)
    }
                            ...
                            (items.map((item, i) => {
                                return (
                                    <NftBox key={i} onClick={() => handleShowDetailClick(item)}>
                                        <NftImage src={item?.nft?.image || NoImageIcon} hasImage={item?.nft?.image} />
                                        <Attrs>
                                            <PropertyName>{item?.nft?.name || item?.nft?.propertyId}</PropertyName>
